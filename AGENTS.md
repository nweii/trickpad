...
The edge also caches a 404 for a URL that does not exist yet, and any probe of an unpublished URL plants one — including the publish preview's own occupancy check, which runs seconds before upload and verification. The zone must carry an edge-TTL rule that stores no 404 from the updates host; without it, first publication of every new archive fails verification against its own cached miss, and the recovery is a per-URL cache purge.
...
**`CHANGELOG.md` is an interface, not only prose.** A downstream surface parses it to render a customer-facing changelog and the version it advertises. The shape it relies on: `## X.Y.Z` per release, a `Released YYYY-MM-DD.` line, a `### Section` heading per group, `- ` bullets that may wrap, and any trailing paragraph as closing notes. Entries are written in the imperative, so the section heading supplies the tense. Changing that structure degrades a page outside this repository, which no check here will catch. Unreleased entries may be staged under `## Unreleased` above the newest release. The website ignores everything above the first version heading, so the section stays private until the release renames it to `## X.Y.Z` and adds its `Released` line.
...
`experimental-mouse-click-gestures` has no effect and is accepted only so an existing `config.toml` that sets it keeps loading without a report. Magic Mouse two- and three-finger physical-click bindings load unconditionally.
...
Record Candidate Gesture prototypes a motion that has no recognizer. It asks for a short candidate name and a repetition count, then runs one guided step per repetition with the same countdown, tone, capture window, and Clean/Botched/Unsure/Skip labels as the other protocols. Nothing is evaluated or dispatched against the candidate, so the data is raw trackpad and Magic Mouse contact frames plus the execution-quality labels. The typed name reaches the bundle manifest only, and the analyzer reports frame count, peak contact count, span, onset spread, and duration for each repetition.
...
`KeyUtility.m` also renames upstream's `simulateSpecialKey:` to `simulateSystemKey:`, the dispatch path for the media, volume, brightness, and keyboard-backlight actions. `Gesture.m` renames its unreachable upstream dispatch strings `"Next"` and `"Previous"` to `"Next Track"` and `"Previous Track"`, which `actionNames` in `Config.m` now targets. An upstream sync that restores either old name breaks the build at link time or silently strands those actions.
...
`TrackpadInteraction.m` classifies each trackpad contact sequence before the tap recognizers claim it. Broad palm contacts and physical clicks reject tap-only gestures, one recognizer may claim a sequence, and raw full lift resets the state. Physical clicks use the filtered contact count already observed by the module, retain its peak through a brief missing-contact frame, and reject the configured action when the CG event stream becomes a drag. Physical mouse-up clears the peak so the next click cannot inherit it. Keep gesture-specific geometry and timing in `Gesture.m`; shared eligibility and inter-gesture arbitration belong in this module. Three contact-count flavors exist, and a decision must name which one it reads: the raw count (sequence lifecycle and full-lift reset), the thumb-filtered count the recognizers receive, and the fingertip-scale count that excludes palm-scale contacts, which feeds the swipe family's scroll suppression so a resting palm cannot arm it. A fourth distinction sits beneath all three: the hardware also reports hovering and out-of-range contacts, and every flavor above counts only active touches — interaction ownership and five-finger eligibility read hover-stripped contacts, so a finger lingering above the surface neither holds a sequence open nor joins a count. An attempt to make these views explicit as immutable per-frame snapshots (the closed refactor behind the `refactor/contact-frames` branch) failed twice on view-routing regressions; any retry needs a replay oracle that exercises hover, edge, thumb, and left-handed cases before consumers move.
...
`MultitouchDeviceLifecycle.m` owns every conversation with `MultitouchSupport.framework` about devices: enumeration, callback registration, stop, reload, the device-add retry timer, and the post-wake rebuild. Gesture recognition receives contact frames through its two callbacks and nothing else, so a macOS release that shifts the framework is mended in this one module. It deliberately carries no health or garbage detection — a frames-since-launch counter cannot distinguish idle hardware from framework failure. `MultitouchDeviceLifecycleCheck.m` holds its check, including the rebuild ordering the wake-recovery guard in `scripts/check.sh` relies on.
