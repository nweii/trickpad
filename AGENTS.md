...
It asserts keystroke parsing, app scopes, exclusions, expanded options, action and deferred dispatch, URL substitution parsing and resolution, script validation and execution, skipped bad lines, boolean spellings, and comment stripping. It also asserts the held middle-button lifecycle: one press produces one down and one up, a second gesture cannot press a held button, and the engine releases the button on every reset while a tap keeps the momentary pair. It also asserts that every slug appears in both `GESTURES.md` and `config-notes.default.md`, and that every engine name reachable from a slug has a menu phrase. Adding a gesture without documenting it fails the check. Direct gesture dispatch is allowlisted so a recognizer that bypasses contact-sequence ownership also fails the check.
...
`MiddleButtonLifecycle.m` owns the one held middle-button press either device can start, so a Magic Mouse click, a Magic Mouse thumb rest, and a trackpad physical click press the button through the same state rather than three lifecycles. A configured physical click bound to `middle-click` presses the button in place of the native mouse-down on both devices, the drags and the release that follow are rewritten from that held state, and its mouse-up dispatches no action because the press already delivered it. Taps keep the momentary down-and-up pair in `doCommand`. Every path that abandons a press ends it through `releaseHeldMiddleButton`: turning either device off, the reset a reload and wake run, and a mouse-down arriving while a press is still held. No path can leave the button down.
