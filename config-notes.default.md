...
- Set `inherit = false` in an application table when that app must ignore every device-global binding. Bindings in the application table still apply. Set it separately for the mouse and trackpad.
- Add `fn_` before a gesture name to make that binding run only while the physical Fn key is held. `function_` is an alias. Quoted `"fn+gesture"` and `"function+gesture"` keys are equivalent. Prefer `fn_` when writing a new binding. The same gesture does not need an ordinary binding.
...
Some useful app commands have no shortcut. A `menu:` binding chooses the command directly; put it in that application's table unless the command is common to many applications. A global menu binding shows a failure popover in every application that lacks the command, so reserve global menu bindings for commands such as `Window > Minimize` or `Edit > Copy`. A macOS App Shortcut is the alternative when the user also wants a keyboard shortcut. Confirm the exact menu titles with the user or in the target app before binding them. Do not automate System Settings for this because its interface and menu titles can change.
...
- No keyboard equivalent. Opening a URL or deep link, running a script, or adding a logical mouse button has no keystroke to save, so a gesture is a natural fit. A physical click bound to a keystroke holds its keys until release. A physical click bound to `middle-click` or `mouse-3` through `mouse-32` holds that button through a drag. A tap sends one press and release.
