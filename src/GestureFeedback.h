// Shows a short message from Trickpad in a popover below its menu bar icon,
// without activating Trickpad or taking keyboard focus from the application
// in use.

#import <AppKit/AppKit.h>

// Sets the menu bar item the popover points at. Pass nil when the icon is
// hidden; messages are then dropped. Call on the main thread.
void MGGestureFeedbackSetAnchor(NSStatusItem *item);

// Where a click on the popover takes the user: the place the fix belongs.
...
// only closes the popover. Safe to call from any thread.
